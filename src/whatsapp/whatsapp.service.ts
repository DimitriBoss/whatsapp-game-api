...
export class WhatsappService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleDestroy() {
    ...
    try {
      ...
      this.logger.error(
        '⚠️ Erreur lors de la fermeture du client WhatsApp :',
        error,
      );
    }
  }
  ...
  private initializeClient() {
    ...
    this.client.on('qr', (qr) => {
      this.currentQr = qr;
      this.logger.log(
        '📱 Scanne ce QR Code avec ton WhatsApp pour lier le bot :',
      );
    ...
    this.client.on('message', async (msg) => {
      try {
        ...
        this.logger.log(
          `📩 Message reçu de [${senderNumber}] dans [${isGroup ? 'groupe' : 'solo'}] : "${msg.body}"`,
        );
        ...
        const groupSession = isGroup
          ? await this.gameService.getGameSession(msg.from)
          : null;
        const groupGames = [
          'devinette',
          'pendu',
        ...
        if (
          isGroup &&
          groupSession &&
          groupSession.currentGame &&
          groupGames.includes(groupSession.currentGame)
        ) {
          const groupGameData = groupSession.gameData as any;
          if (groupGameData?.mode === 'challenge') {
            const isParticipant =
              senderNumber === groupGameData.player1 ||
              senderNumber === groupGameData.player2;
            if (!isParticipant) {
              const isGlobalCmd =
                text === '/score' ||
                text === '/points' ||
          ...
          sessionKey = msg.from;
          if (groupSession.currentGame === 'devinette')
            botState = BotState.PLAYING_DEVINETTE;
          else if (groupSession.currentGame === 'pendu')
            botState = BotState.PLAYING_PENDU;
          else if (groupSession.currentGame === 'quiz')
            botState = BotState.PLAYING_QUIZ;
          else if (groupSession.currentGame === 'emoji')
            botState = BotState.PLAYING_EMOJI;
          else if (groupSession.currentGame === 'group_setup')
        ...
        const mentionsBot =
          msg.mentionedIds && botWid && msg.mentionedIds.includes(botWid);
        ...
        if (isGroup && !isCommand && !mentionsBot) {
          ...
          switch (botState) {
            ...
            case BotState.PLAYING_DEVINETTE:
              ...
              if (isGroup) {
                const prefixRegex =
                  /^(reponse\s*:\s*|reponse\s+|r\s*:\s*|r\s+|\/r\s*|\/reponse\s*)/i;
                isStrictGameInput =
                  text === '0' ||
                  text === 'suivant' ||
                  prefixRegex.test(msg.body.trim());
              } else {
                isStrictGameInput = true;
              }
              break;
            ...
            case BotState.PLAYING_PENDU:
              ...
              if (isGroup) {
                const prefixRegex =
                  /^(reponse\s*:\s*|reponse\s+|r\s*:\s*|r\s+|\/r\s*|\/reponse\s*)/i;
                isStrictGameInput =
                  text === '0' || prefixRegex.test(msg.body.trim());
              } else {
                isStrictGameInput = true;
              }
              break;
        ...
        if (
          isGroup &&
          (text.startsWith('/morpion') || text.startsWith('#morpion'))
        ) {
          if (msg.mentionedIds && msg.mentionedIds.length > 0) {
            const targetId = msg.mentionedIds[0];
            if (targetId === senderNumber) {
              await msg.reply(
                `⚠️ Tu ne peux pas te défier toi-même au Morpion !`,
              );
              return;
            }
        ...
        switch (botState) {
          ...
          case BotState.MAIN_MENU:
            if (session?.currentGame === 'group_setup') {
              ...
              if (text === '0' || text === 'retour' || text === 'quitter') {
                if (setupData?.mode === 'awaiting_challenge_mention') {
                  ...
                  await msg.reply(
                    await this.getMainMenuMessage(senderNumber, TIP),
                  );
                }
                return;
              }
            ...
            if (text === '1') {
              ...
              if (!question) {
                ...
                const totalPlayed =
                  await this.gameService.incrementUserQuestionCount(
                    senderNumber,
                    QuestionType.DEVINETTE,
                  );
              ...
              await this.gameService.updateUserState(
                senderNumber,
                BotState.PLAYING_EMOJI,
              );
              const emojiWelcome =
                await this.emojiService.startGame(sessionKey);
          ...
          case BotState.PLAYING_ACTION_VERITE:
            if (
              ...
              if (currentQ?.type === QuestionType.ACTION) {
                ...
                const isConfirmation = confWords.some((word) =>
          ...
          case BotState.PLAYING_DEVINETTE:
            if (
              ...
              if (!session || !session.currentQuestionId) {
                await msg.reply(
                  `🧩 Tu n'as pas de devinette active. Envoie *suivant* pour en obtenir une.`,
                );
              } else {
                ...
                if (!question || !question.answer) {
                  await this.gameService.updateGameSession(sessionKey, null, 0);
                  await msg.reply(
                    `🧩 Erreur de configuration de la devinette. Envoie *suivant* pour passer.`,
                  );
                } else {
                  const prefixRegex =
                    /^(reponse\s*:\s*|reponse\s+|r\s*:\s*|r\s+|\/r\s*|\/reponse\s*)/i;
                  ...
                  const normalizedUserAnswer =
                    this.normalizeText(cleanGuessText);
                  ...
                  if (isCorrect) {
                    ...
                    const newPoints =
                      await this.gameService.incrementUserPoints(
                        senderNumber,
                        10,
                      );
                    await msg.reply(
                      `🎉 *Félicitations !* C'est la bonne réponse ! 🥳\n` +
                        ...
                        `_(Envoie *2* pour une autre devinette, ou *0* pour le menu)_` +
                        TIP,
                    );
                  } else {
          ...
          case BotState.PLAYING_PENDU:
            if (
              ...
              const response = await this.penduService.handleGuess(
                sessionKey,
                senderNumber,
                cleanGuessText,
                session?.gameData,
              ...
              if (isGroup) {
                const updatedSession =
                  await this.gameService.getGameSession(sessionKey);
          ...
          case BotState.PLAYING_QUIZ:
            if (
              ...
              const response = await this.quizService.handleAnswer(
                sessionKey,
                senderNumber,
                cleanGuessText,
                session?.gameData,
          ...
          case BotState.PLAYING_EMOJI:
            if (
              ...
              const response = await this.emojiService.handleGuess(
                sessionKey,
                senderNumber,
                cleanGuessText,
                session?.gameData,
          ...
          case BotState.PLAYING_MORPION:
            ...
            if (isExitCommand) {
              ...
              if (isGroup && text.startsWith('1')) {
                ...
                const response = await this.morpionService.startSoloGame(
                  sessionKey,
                  'HARD',
                );
                ...
                const result =
                  await this.morpionService.createMultiplayerGame(senderNumber);
                const game = await this.prisma.morpionGame.findUnique({
                  where: { inviteCode: result.code },
                });
                await this.gameService.updateGameSessionWithData(
                  sessionKey,
                  'morpion',
              ...
              const result = await this.morpionService.joinMultiplayerGame(
                text.toUpperCase(),
                senderNumber,
              );
              if (!result) {
                ...
                // Alerter l'hôte
                await this.client.sendMessage(
                  result.game.player1ChatId,
                  result.messagePlayer1,
                );
                // Alerter le joueur actuel
                await msg.reply(result.messagePlayer2);
              }
            } else if (gameData.mode === 'solo') {
              const response = await this.morpionService.handleSoloMove(
                sessionKey,
                senderNumber,
                text,
                gameData,
              ...
              if (!result.success) {
                ...
                if (isGroup) {
                  ...
                  await msg.reply(result.message);
                  if (
                    result.shouldNotifyAdversary &&
                    result.adversaryChatId &&
                    result.adversaryMessage
                  ) {
                    await this.client.sendMessage(
                      result.adversaryChatId,
                      result.adversaryMessage,
        ...
        this.errorLogService.logError(
          'whatsapp.service.message_handler',
          error,
          Severity.HIGH,
        );
        try {
  ...
  private async handleAdminCommand(msg: any, senderNumber: string) {
    ...
    if (!isAdmin) {
      await msg.reply(
        `⚠️ Tu n'as pas l'autorisation d'exécuter des commandes d'administration.`,
      );
      return;
    }
    ...
    const command =
      spaceIndex === -1
        ...
        : commandLine.substring(0, spaceIndex).toLowerCase();
    const arg =
      spaceIndex === -1 ? '' : commandLine.substring(spaceIndex + 1).trim();
    ...
    try {
      if (command === 'stats') {
        ...
        if (!arg) {
          await msg.reply(
            `⚠️ Spécifie le numéro à bloquer. Exemple : *#block 229XXXXXXXX*`,
          );
          return;
        }
        ...
        await msg.reply(
          `✅ L'utilisateur *${target.split('@')[0]}* a été bloqué.`,
        );
        await this.adminService.logAdminAction(
          senderNumber,
          'WHATSAPP_BLOCK',
        ...
        if (!arg) {
          await msg.reply(
            `⚠️ Spécifie le numéro à débloquer. Exemple : *#unblock 229XXXXXXXX*`,
          );
          return;
        }
        ...
        await msg.reply(
          `✅ L'utilisateur *${target.split('@')[0]}* a été débloqué.`,
        );
        await this.adminService.logAdminAction(
          senderNumber,
          'WHATSAPP_UNBLOCK',
        ...
        if (!arg) {
          await msg.reply(
            `⚠️ Saisis le message à diffuser. Exemple : *#broadcast Salut à tous !*`,
          );
          return;
        }
        ...
        await msg.reply(
          `📢 Lancement de la diffusion vers *${users.length}* utilisateurs...`,
        );
        ...
        for (const user of users) {
          try {
            await this.client.sendMessage(
              user.phoneNumber,
              `📢 *DIFFUSION ADMIN GameBot* 📢\n\n${arg}`,
            );
            successCount++;
            await new Promise((resolve) => setTimeout(resolve, 200));
          } catch (e) {
            this.logger.warn(
              `Échec de l'envoi broadcast à ${user.phoneNumber} : ${e.message}`,
            );
          }
        }

        await msg.reply(
          `🏁 Diffusion terminée. Succès : *${successCount}/${users.length}*`,
        );
        await this.adminService.logAdminAction(
          senderNumber,
          'WHATSAPP_BROADCAST',
        ...
        if (!validTypes.includes(qTypeStr)) {
          await msg.reply(
            `⚠️ Type de question invalide. Doit être : ACTION, VERITE, DEVINETTE ou QUIZ`,
          );
          return;
        }
        ...
        await msg.reply(
          `✅ Question ajoutée avec succès ! ID : *${question.id}*`,
        );
        await this.adminService.logAdminAction(
          senderNumber,
          'WHATSAPP_ADDQUESTION',
          question.id,
        );
      } else {
        await msg.reply(
          `⚠️ Commande admin inconnue. Commandes valides : #stats, #block, #unblock, #broadcast, #addquestion`,
        );
      }
    } catch (err) {
      this.errorLogService.logError(
        `whatsapp.service.admin_command.${command}`,
        err,
        Severity.HIGH,
      );
      await msg.reply(
  ...
  private async getMainMenuMessage(
    senderNumber: string,
    tip: string,
  ): Promise<string> {
    const user = await this.gameService.getOrCreateUser(senderNumber);
